...
mergeddata_urv <- subset(mergeddata_urv, !(mergeddata_urv$s %in% exclude_othEUR$V1)) 
...
#mergeddata_urv <- subset(mergeddata_urv[,c("s","phenotype.SEX","outcome",colnames(mergeddata_urv)[grep("n_URV_PTV|n_URV_synonymous|pca.PC",colnames(mergeddata_urv))])])
...
write.table(dv, file = "/media/veracrypt10/Analyses/annotations/scz_urv_ptv_geneset_estimates_othEUR_exclusions.tsv", sep="\t",dec = ".", row.names = F )

#URV synonymous variants####
...
write.table(dv, file = "/media/veracrypt10/Analyses/annotations/scz_urv_syn_geneset_estimates_othEUR_exclusions.tsv", sep="\t",dec = ".", row.names = F )
