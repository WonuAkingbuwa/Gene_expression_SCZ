...
ptv_tot_estimates <- fread("scz_urv_ptv_geneset_estimates_ewptvcov.tsv", sep='\t', header=TRUE, data.table=FALSE)
ptv_tot_estimates$geneset <- gsub("n_URV_PTV__URVs_|_intervals_bed_tsv", "", ptv_tot_estimates$geneset)
ptv_tot_estimates <- ptv_tot_estimates[,c(2:9)]
colnames(ptv_tot_estimates)[2:8] <- paste("tot", colnames(ptv_tot_estimates[,c(2:8)]), sep = "_")

ptv_FIN_estimates <- fread("scz_urv_ptv_geneset_estimates_FIN_exclusions.tsv", sep='\t', header=TRUE, data.table=FALSE)
ptv_FIN_estimates$geneset <- gsub("n_URV_PTV__URVs_|_intervals_bed_tsv", "", ptv_FIN_estimates$geneset)
ptv_FIN_estimates <- ptv_FIN_estimates[,c(2:9)]
colnames(ptv_FIN_estimates)[2:8] <- paste("fin", colnames(ptv_FIN_estimates[,c(2:8)]), sep = "_")

ptv_NorSwe_estimates <- fread("scz_urv_ptv_geneset_estimates_NorSwe_exclusions.tsv", sep='\t', header=TRUE, data.table=FALSE)
ptv_NorSwe_estimates$geneset <- gsub("n_URV_PTV__URVs_|_intervals_bed_tsv", "", ptv_NorSwe_estimates$geneset)
...
ptv_othEUR_estimates <- fread("scz_urv_ptv_geneset_estimates_othEUR_exclusions.tsv", sep='\t', header=TRUE, data.table=FALSE)
ptv_othEUR_estimates$geneset <- gsub("n_URV_PTV__URVs_|_intervals_bed_tsv", "", ptv_othEUR_estimates$geneset)
...
ptv_allEURout_estimates <- fread("scz_urv_ptv_geneset_estimates_allEURout_exclusions.tsv", sep='\t', header=TRUE, data.table=FALSE)
ptv_allEURout_estimates$geneset <- gsub("n_URV_PTV__URVs_|_intervals_bed_tsv", "", ptv_allEURout_estimates$geneset)
...
#plot
tiff("pca_exclusions_plot_urv_ptv.tiff", width = 3000, height = 2000, res=600)
