...
if (perform_plotting) {
    for (i in PCs) {
        aes <- aes_string(x=paste0('PC',i), y=paste0('PC',i+1), color='phenotype.PRIMARY_DISEASE')
        ...
        aes <- aes_string(x=paste0('PC',i), y=paste0('PC',i+1), color='phenotype.SITE')
...
if (perform_plotting) {
    for (i in PCs) {
        ...
        aes <- aes_string(x=paste0('PC', i), y=paste0('PC', i+1), color='SAMP_SOURCE')
        ...
        aes <- aes_string(x=paste0('PC', i), y=paste0('PC', i+1), color='SITE')
...
df_train = filter(df_1kg, PRIMARY_DISEASE=='1KG') %>%
  select(c(SUPER_POPULATION, POPULATION, PC1, PC2, PC3, PC4, PC5, PC6, PC7, PC8, PC9, PC10))

df_predict = filter(df_1kg, PRIMARY_DISEASE!='1KG') %>%
...
fwrite(df_sample_PCs, file='Analyses/sample_count_pcs.tsv', quote=FALSE, row.names=FALSE, col.names=FALSE, sep='\t')
